...
export default async (req, res) => {
    ...
    switch (method) {
        case 'POST': {
            ...
            const { id, command } = getBody(req?.body);

            if (!auth_token || !id || !command) return response.sendError('Invalid request.'); 
            ...
            const { data: { rows: [{ host, port, api_key }] } } = await selectInTable(tables.fivemServers, 'host, port, api_key', [
                { name: 'id', value: id }
            ]);
            ...
            try {
                execute = await (await (await fetch(`http://${host}:${port}/${api_key}/execute`, {
                    method: 'POST',
            ...
            if (!execute) return response.sendError('Failed to execute command.');

...
export default async (req, res) => {
    ...
    switch (method) {
        case 'POST': {
            ...
            const { exists: hostExists } = await selectInTable(tables.linuxServers, null, [
            ...
            const { error: failedServerCreation } = await insertIntoTable(tables.linuxServers, [
        ...
        case 'DELETE': {
            ...
            const { exists: serverExists } = await selectInTable(tables.linuxServers, null, [
            ...
            const { error: failedServerDeletion } = await deleteFromTable(tables.linuxServers, [
                { name: 'id', value: id, seperator: 'AND' },
                { name: 'owner_id', value: user?.id }
            ]);
        ...
        case 'PUT': {
            ...
            const { error: failedServerUpdate } = await updateInTable(tables.linuxServers, [
                { name: 'nickname', value: nickname },
                { name: 'host', value: host },
                { name: 'port', value: port },
                { name: 'api_key', value: api_key },
